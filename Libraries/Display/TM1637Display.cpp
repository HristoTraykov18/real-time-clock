...
void TM1637Display::showNumberDec(int num, bool leadingZero, uint8_t length, uint8_t pos)
{
  showNumberDecEx(num, 0, leadingZero, length, pos);
}

void TM1637Display::showNumberDecEx(int num, uint8_t dots, bool reversedDigitLogic, bool leadingZero, uint8_t length, uint8_t pos)
{
  if (reversedDigitLogic)
    for(uint8_t i = 0; i < sizeof(digitToSegment); i++)
      digitToSegment[i] = digitToSegmentReversed[i];
  uint8_t digits[4];
